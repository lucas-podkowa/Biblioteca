import { pool } from "../config/db.js";
import "dotenv/config";

const bookModel = {
  getAll: async (busqueda = "") => {
    try {
      let query = "SELECT * FROM libro";
      const params = [];

      if (busqueda) {
        query += " WHERE titulo ILIKE $1 OR autor ILIKE $2";
        params.push(`%${busqueda}%`, `%${busqueda}%`);
      }

      const { rows } = await pool.query(query, params);

      //------------------------  OPCIONAL ---------------
      // //agregar la URL completa si existe imagen
      const books = rows.map((book) => ({
        ...book,
        image_url: book.image_url
          ? `${process.env.BASE_URL}${book.image_url}`
          : null,
      }));

      return books;
      // //------------------------  OPCIONAL ---------------

      //return rows;
    } catch (error) {
      console.error("Error fetching books:", error);
      throw new Error("Could not fetch books from the database.");
    }
  },

  getById: async (id) => {
    try {
      const query = "SELECT * FROM libro WHERE id_libro = $1";
      const { rows } = await pool.query(query, [id]);
      if (rows.length === 0) {
        return null;
      }
      return rows[0];
    } catch (error) {
      console.error(`Error fetching book with id ${id}:`, error);
      throw new Error(`Could not fetch book with id ${id} from the database.`);
    }
  },

  //agregar existencias
  create: async (book) => {
    try {
      console.log(book);
      const { titulo, autor, editorial, anio_publicacion, genero, image_url } =
        book;
      const query =
        "INSERT INTO libro (titulo, autor, editorial, anio_publicacion, genero, image_url) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id_libro";
      const { rows } = await pool.query(query, [
        titulo,
        autor,
        editorial,
        anio_publicacion,
        genero,
        image_url,
      ]);
      return { id: rows[0].id_libro, ...book };
    } catch (error) {
      console.error("Error creating book:", error);
      throw new Error("Could not create book in the database.");
    }
  },
  updateById: async (id, book) => {
    try {
      const {
        titulo,
        autor,
        editorial,
        anio_publicacion,
        genero,
        existencias,
      } = book;
      const query =
        "UPDATE libro SET titulo = $1, autor = $2, editorial = $3, anio_publicacion = $4, genero = $5, existencias = $6 WHERE id_libro = $7";
      const result = await pool.query(query, [
        titulo,
        autor,
        editorial,
        anio_publicacion,
        genero,
        existencias,
        id,
      ]);
      return result.rowCount > 0;
    } catch (error) {
      console.error(`Error updating book with id ${id}:`, error);
      throw new Error(`Could not update book with id ${id} in the database.`);
    }
  },
  getAvailable: async () => {
    try {
      const query = "SELECT * FROM libro WHERE existencias > 0";
      const { rows } = await pool.query(query);
      return rows;
    } catch (error) {
      console.error("Error fetching available books:", error);
      throw new Error("Could not fetch available books from the database.");
    }
  },
  deleteById: async (id) => {
    try {
      const query = "DELETE FROM libro WHERE id_libro = $1";
      const result = await pool.query(query, [id]);
      return result.rowCount > 0;
    } catch (error) {
      console.error(`Error deleting book with id ${id}:`, error);
      throw new Error(`Could not delete book with id ${id} from the database.`);
    }
  },
  updateStockById: async (id, existencias) => {
    try {
      const query = "UPDATE libro SET existencias = $1 WHERE id_libro = $2";
      const result = await pool.query(query, [existencias, id]);
      return result.rowCount > 0;
    } catch (error) {
      console.error(`Error updating stock for book with id ${id}:`, error);
      throw new Error(
        `Could not update stock for book with id ${id} in the database.`
      );
    }
  },
};

export default bookModel;
